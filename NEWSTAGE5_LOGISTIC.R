############################################################
# NEW STAGE-5 TREATMENT-SCHEDULE OPTIMISATION — LOGISTIC
############################################################
...
fit_candidates <- c(
  ...
  "results/stage4new_combination_mfull_logistic_reducesum/fit_stage4NEW_log_reducesum.rds",
  "~/Scratch/dissertation/results/stage4new_combination_mfull_logistic_reducesum/fit_stage4NEW_log_reducesum.rds",
  "~/Desktop/STADIO4/FINAL_FITS/stage4new_combination_mfull_logistic_reducesum/fit_stage4NEW_log_reducesum.rds"
)

fit_path <- fit_candidates[file.exists(path.expand(fit_candidates))][1]
if (is.na(fit_path)) {
  stop(
    "Could not find the FINAL NEW mouse-specific-PK Stage-4 Logistic fit. Tried:\n",
...
results_dir <- "results/NEWSTAGE5_logistic"
...
combo_reduced_rhs <- function(t, y, pars) {
  ...
  growth <- pars[["r"]] * P * (1 - V_safe / pars[["K"]])
...
############################################################
# FORWARD SIMULATOR USING FINAL STAGE-4
############################################################
...
############################################################
# THE PRIMARY CANDIDATE SCHEDULES
############################################################
...
############################################################
# RUN PRIMARY SCHEDULES OVER POSTERIOR DRAWS
############################################################
...
############################################################
# RUN DRUG-RT TIMING SWEEP OVER POSTERIOR DRAWS
############################################################
...
# 16A. Primary schedule outcomes: final tumour volume.
p_final <- primary_summary %>%
  ...
  labs(
    ...
    subtitle = paste0("Posterior predictions at day ", FOLLOWUP_DAY, " under the Logistic model"),
    x = NULL,
...
# 16B. Primary schedule outcomes: tumour-volume AUC.
...
metadata <- data.frame(
  ...
  value = c(
    "Full joint Logistic treatment model",
    fit_path,
    data_path,
...
cat("============================================================\n")
cat("Model: Logistic full joint treatment model\n")
