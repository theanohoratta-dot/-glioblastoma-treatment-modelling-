# TREATMENT-SCHEDULE OPTIMISATION
# Posterior forward simulations from the final Stage-4
...
fit_candidates <- c(
  ...
  "results/stage4new_combination_mfull_exp_reducesum/fit_stage4NEW_exp_reducesum.rds",
  "~/Scratch/dissertation/results/stage4new_combination_mfull_exp_reducesum/fit_stage4NEW_exp_reducesum.rds",
  "~/Desktop/STADIO4/FINAL_FITS/stage4new_combination_mfull_exp_reducesum/fit_stage4NEW_exp_reducesum.rds"
)

fit_path <- fit_candidates[file.exists(path.expand(fit_candidates))][1]
if (is.na(fit_path)) {
  stop(
    "Could not find the FINAL NEW mouse-specific-PK Stage-4 Exponential fit. Tried:\n",
...
results_dir <- "results/NEWSTAGE5_exponential"
...
p_final <- primary_summary %>%
  ...
  labs(
    ...
    subtitle = paste0("Posterior predictions at day ", FOLLOWUP_DAY, " under the Exponential model"),
    x = NULL,
...
metadata <- data.frame(
  ...
  value = c(
    "Full joint Exponential treatment model",
    fit_path,
    data_path,
...
cat("============================================================\n")
cat("Model: Exponential full joint treatment model\n")
...
cat("  2. Repeat key comparisons over alternative V0 choices.\n")
cat("  3. Repeat the key schedules under Exponential and Gompertz models.\n")
cat("============================================================\n")


# Observed combination baselines, used to define transparent V0
# sensitivity values 
...
############################################################
# 17. REFINED DRUG-RT TIMING SEARCH
############################################################
