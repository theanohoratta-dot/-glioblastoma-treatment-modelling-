##################################################
# RT-stage : CONTROL + RT-ONLY MICE — LOGISTIC
##################################################
...
results_dir <- "results/stage2_mfull_logistic_ctrl_rt"
figures_dir <- "figures/stage2_mfull_logistic_ctrl_rt"
stan_dir <- "stan"
...
# Extract control + RT-only dataset
...
cat("Baseline/first-RT checks passed for RT and control mice.\n")
...
cat("\nStage 2 data preparation completed successfully.\n")
...
stan_files <- c(
  exponential = file.path(stan_dir, "exponential_rt_MFULL_logistic_build.stan"),
  logistic = file.path(stan_dir, "logistic_rt_MFULL.stan"),
  gompertz = file.path(stan_dir, "gompertz_rt_MFULL_logistic_build.stan")
)
...
mod_rt_logistic <- cmdstan_model(stan_files[["logistic"]])
...
system.time({
  fit_rt_logistic <- mod_rt_logistic$sample(
    data = rt_logistic_stan_data,
    chains = 4,
    parallel_chains = 4,
    iter_warmup = 500,
    iter_sampling = 1000,
    ...
    max_treedepth = 12,
    seed = 123,
    init = rt_logistic_init,
    output_dir = csv_dir,
    output_basename = "logistic_MFULL_ctrl_rt_full",
    refresh = 50
  )
}) -> full_timing

print(full_timing)
print(fit_rt_logistic$diagnostic_summary())

fit_rt_logistic$save_object(
  file.path(results_dir, "fit_rt_logistic_MFULL_ctrl_rt_full.rds")
)

cat("\nMFULL LOGISTIC CONTROL + RT FULL FIT COMPLETE\n")
...
if (have_all_full_fits) {
  cat("\nAll three completed Stage 2 fits are available.\n")
} else if (RUN_DOWNSTREAM_ANALYSIS) {
  stop(
    "RUN_DOWNSTREAM_ANALYSIS=TRUE but all three completed full fits were not found."
  )
} else {
  cat("\nFull fits not all available; downstream analysis is disabled.\n")
}
...
write_model_summaries <- function(fit, model_name, include_K = FALSE) {
  ...
  if (include_K) {
    vars <- c(vars, "K_pop", "logK_pop", "sigma_logK")
  }
...
if (have_all_full_fits && RUN_DOWNSTREAM_ANALYSIS) {
  ...
  cat("\nSection 19A residual diagnostics complete.\n")
